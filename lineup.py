...
def get_all_teams_with_details():
    """Get all teams with additional details"""
    session = database.get_db_session()
    try:
        teams = session.query(database.Team).all()
        team_details = []
        for team in teams:
            # You could add player count and other statistics here
            team_details.append({
                "id": team.id, 
                "name": team.name,
                "league": team.league,
...
def generate_game_plan_pdf(team_id, game_number):
    ...
    # Add game header
    elements.append(Paragraph(f"Game {game_number} Plan", title_style))
    
    # Add game information with validation
    opponent = game_info.get("Opponent", "Unknown")
    elements.append(Paragraph(f"Opponent: {opponent}", normal_style))
    ...
    try:
        ...
        elements.append(Paragraph("Date: Not scheduled", normal_style))
    ...
    for i, jersey in enumerate(batting_order, 1):
        if jersey in jersey_to_player:
            ...
            jersey_num = player.get("Jersey Number", "")
        ...
        if inning_key in fielding_data:
            ...
            for jersey, position in fielding_data[inning_key].items():
                if jersey in jersey_to_player:
                    ...
                    # Determine position order with validation
                    try:
                        position_order = POSITIONS.index(position) if position in POSITIONS else 999
            ...
            for pos in sorted(position_assignments, key=lambda x: x.get("Order", 999)):
                # Only include non-bench positions in the detailed layout
                if pos.get("Position") != "Bench":
                    positions_data.append([pos.get("Position", ""), pos.get("Player", ""), pos.get("Jersey", "")])
            
            # Add bench players at the end in a single row
            bench_players = [pos for pos in position_assignments if pos.get("Position") == "Bench"]
            if bench_players:
                bench_names = ", ".join([f"{p.get('Player', '')} {p.get('Jersey', '')}" for p in bench_players])
...
def generate_fielding_rotation(data):
    ...
    6. STRICTLY BALANCE playing time:
       - Every available player should have nearly equal infield time (within 1 inning difference). Infield positions are: Pitcher, 1B, 2B, 3B, SS.
       - Every available player should have nearly equal outfield time (within 1 inning difference). Outfield positions are: Catcher, LF, RF, LC, RC.
    ...
    7. DOUBLE CHECK that ALL of these positions are assigned in EVERY inning: {', '.join(required_positions)}
    ...
    try:
        ...
        # Verify all required positions are filled in each inning
        valid_plan = True
        missing_positions = {}
        ...
        for inning, positions in fielding_plan.items():
            if not isinstance(positions, dict):
                valid_plan = False
                if inning not in missing_positions:
                    missing_positions[inning] = []
                missing_positions[inning].append("Invalid position data (not a dictionary)")
                continue
            ...
            for req_pos in required_positions:
                if req_pos not in field_positions:
                    ...
                    if inning not in missing_positions:
                        missing_positions[inning] = []
                    missing_positions[inning].append(req_pos)
            ...
            for pos, count in position_counts.items():
                if count > 1:
                    ...
                    if inning not in missing_positions:
                        missing_positions[inning] = []
                    missing_positions[inning].append(f"{pos} (duplicate)")
        
        if not valid_plan:
            # If validation fails, return the error details
            error_msg = "Validation failed: "
            for inning, positions in missing_positions.items():
                error_msg += f"{inning}: missing or duplicate {', '.join(positions)}; "
                
            return {"error": error_msg.strip()}, 400
    ...
    with gen_col:
        ...
        if st.button("Generate Field Positions", key="generate_fielding_ai"):
            ...
            with st.spinner("Generating balanced fielding rotation..."):
                ...
                if not valid_plan:
                    st.error("Invalid fielding plan. Details:")
                ...
                # Success message
                st.success("Fielding plan generated! Review the plan below.")
...
def main():
    ...
    if st.session_state.team_id is None:
        ...
        if st.sidebar.button("Create Example Team"):
            ...
            team_id = db.create_team(team_info)
    ...
    # Tab 9: Data Management / Team Management
        ...
        # Show current teams
        teams = get_all_teams_with_details()
        if teams:
        ...
        with st.form("create_team_form_management"):
            ...
            if submit_new_team:
                if not new_team_name:
                    ...
                    if st.button("Switch to new team now"):
